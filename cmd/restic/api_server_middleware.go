...
// This code is derived from https://github.com/letsencrypt/boulder/blob/master/bdns/dns.go

func parseCidr(network string, comment string) net.IPNet {
	_, net, err := net.ParseCIDR(network)
	if err != nil {
		panic(Sprintf("error parsing %s (%s): %s", network, comment, err))
	}
	return *net
}
...
// OnlyLocal is middleware to guarantee that a request only
// is only handled when it is comes from the local network
func OnlyLocal(next http.Handler) http.Handler {
	isPrivate := func(address string) bool {
		remoteIP := net.ParseIP(address)
		return isPrivateV4(remoteIP) || isPrivateV6(remoteIP)
	}

	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if isPrivate(r.RemoteAddr) {
			http.Error(w, "Only requests from the local network are permitted", http.StatusForbidden)
			return
		}
		next.ServeHTTP(w, r)
	})
